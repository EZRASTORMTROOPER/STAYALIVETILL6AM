...
export class Game {
  ...
  openSecurityCameraRoom(roomId) {
    const roomIndex = this.securityRooms.findIndex((room) => room.id === roomId);
    if (roomIndex < 0 || this.state !== 'running') return;
    this.securityCameraActive = true;
    this.activeSecurityRoomIndex = roomIndex;
    this.updateBackgroundForView();
    this.updateCameraHud();
  }
  ...
  onCameraMapClick(event) {
    ...
    this.openSecurityCameraRoom(roomButton.dataset.cameraRoom);
  }
